...
#ifdef CONFIG_MTRACE
  log_write("<mtrace>: read %d bytes from %x\n", len, addr);
#endif
...
#ifdef CONFIG_MTRACE
  log_write("<mtrace>: write %d bytes data %lx from %x\n", len, data, addr);
#endif
...
void paddr_write(paddr_t addr, int len, word_t data) {
  if(addr == 0x83015740) printf("write data: %llx, len = %d",(unsigned long long)data,len);
